...
.DESCRIPTION
    ...
    Installazione (repo Public):
        irm https://raw.githubusercontent.com/davraf-amuro/dr-dotnet-backend/main/dr-dotnet-backend-install.ps1 | iex

    Installazione (repo Private, con gh autenticato):
        & ([scriptblock]::Create((gh api repos/davraf-amuro/dr-dotnet-backend/contents/dr-dotnet-backend-install.ps1 -H "Accept: application/vnd.github.raw")))

    Aggiornamento (sovrascrive i file gia presenti):
        & ([scriptblock]::Create((irm https://raw.githubusercontent.com/davraf-amuro/dr-dotnet-backend/main/dr-dotnet-backend-install.ps1))) -Update
